...
parser.add_argument('--gril_graphs', default=True, type=bool,
                    help='whether to use gril graphs or point clouds')
parser.add_argument('--num_center_pts', default=36, type=int,
                    help='the number of center points to compute zz_gril')
parser.add_argument('--use_only_lambda_0', default=False, type=int,
                    help='whether to use only lambda 0 or use both lambda 0 and lambda 1 for zz_gril')
...
def main_work(args):
    ...
    use_only_lambda_0 = args.use_only_lambda_0
    
    if use_only_lambda_0:
        gril_vec_len = num_center_pts * 3
    else:
        gril_vec_len = num_center_pts * 6

    # dataset
    train_loader, val_loader, num_nodes, seq_length, num_classes, gril_train, gril_val = get_default_train_val_test_loader(args)
    ...
    model = GNNStack(gnn_model_type=args.arch, num_layers=args.num_layers, 
                     ...
                     seq_len=seq_length, num_nodes=num_nodes, num_classes=num_classes, gril_vec_len=gril_vec_len)
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        acc_train_per, loss_train_per = train(train_loader, model, criterion, optimizer, lr_scheduler, args, gril_train)
        ...
        acc_val_per, loss_val_per = validate(val_loader, model, criterion, args, gril_val)
...
def train(train_loader, model, criterion, optimizer, lr_scheduler, args, gril_train):
    ...
    for count, (data, label) in enumerate(train_loader):
        ...
        gril_train_iter = iter(gril_train)
        gril = next(x for j,x in enumerate(gril_train_iter) if j == count)
        gril = gril[0].cuda(args.gpu).type(torch.float)
        # compute output
        output = model(data, gril)
...
def validate(val_loader, model, criterion, args, gril_val):
    losses = AverageMeter('Loss', ':.4e')
    ...
    with torch.no_grad():
        for count, (data, label) in enumerate(val_loader):
            ...
            gril_val_iter = iter(gril_val)
            gril = next(x for j,x in enumerate(gril_val_iter) if j == count)
            gril = gril[0].cuda(args.gpu).type(torch.float)
            # compute output
            output = model(data, gril)
