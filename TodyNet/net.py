...
class GNNStack(nn.Module):
    ...
    def __init__(self, gnn_model_type, num_layers, groups, pool_ratio, kern_size, 
                 ...
                 seq_len, num_nodes, num_classes, gril_vec_len, dropout=0.5, activation=nn.ReLU()):
        ...
        self.linear2 = nn.Linear(gril_vec_len, gril_vec_len)
        
        self.linear = nn.Linear(heads * out_dim + gril_vec_len, num_classes)
    ...
    def forward(self, inputs: Tensor, gril:Tensor):
        ...
        gril = self.linear2(gril)
        gril = self.activation(gril)
        gril = nn.Dropout(p=self.dropout)(gril)
        out = torch.cat([out, gril], dim=-1)
        out = self.linear(out)
